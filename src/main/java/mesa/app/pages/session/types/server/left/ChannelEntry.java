...
public class ChannelEntry extends HBox implements Styleable {
	...
	public ChannelEntry(SessionPage session, Channel channel) {
		...
		setOnMouseClicked(e-> select(channel));
	...
	public void applyStyle(Style style) {
		...
		backgroundProperty().bind(Bindings.when(selected).then(selectedBack).otherwise(
				Bindings.when(pressedProperty()).then(activeBack).otherwise(
						Bindings.when(active).then(hoverBack).otherwise(Background.EMPTY)
				)
		));
		...
		Consumer<ActionIcon> styleIcon = icon -> {
			...
			icon.fillProperty().bind(Bindings.when(icon.hoverProperty()).then(style.getInteractiveHover()).otherwise(style.getInteractiveNormal()));
		};
